...
Load data
```{r}
data <- read.csv("Data Analysis/dataset/output_test_regression_classes.csv")
...
plot.plotWithDiagonalMarked <- function(confusionMatrix, xlabel, ylabel, xValueLabels, yValueLabels, valueOrdering) {
  ...
  confusionMatrix_diagonalMarked$ref_freq <- confusionMatrix_diagonalMarked$Freq * ifelse(confusionMatrix_diagonalMarked$diag, 1, 4) # flip sign for non-diagonal values
...
```{r}
sentimentValueLabels = c("Super Neg", "Negative", "Positive", "Super Pos")
stanceValueLabels = c("Super Against", "Againts", "Agree", "Super Agree")
valueOrdering = c("1", "2", "3", "4")
```
...
plot
ggsave("images/PredictionSentimentStancereg.png")
```
