...
plot.plotWithDiagonalMarked <- function(confusionMatrix) {
  ...
  confusionMatrix_diagonalMarked$ref_freq <- confusionMatrix_diagonalMarked$ref_freq + max(confusionMatrix_diagonalMarked$ref_freq) - min(confusionMatrix_diagonalMarked$ref_freq) # This for some reason fixes the colour range not being scaled properly, found by experimentation
...
truth_predicted$Reference[truth_predicted$Reference == 0] <- "Neutral"
truth_predicted$Prediction[truth_predicted$Prediction == 0] <- "Neutral"
truth_predicted$Reference[truth_predicted$Reference == 1] <- "Positive"
truth_predicted$Prediction[truth_predicted$Prediction == 1] <- "Positive"
truth_predicted$Reference[truth_predicted$Reference == -1] <- "Negative"
truth_predicted$Prediction[truth_predicted$Prediction == -1] <- "Negative"

truth_predicted$Reference <- as.factor(truth_predicted$Reference)
truth_predicted$Prediction <- as.factor(truth_predicted$Prediction)

confusionMatrix <- caret::confusionMatrix(truth_predicted$Prediction, truth_predicted$Reference)
