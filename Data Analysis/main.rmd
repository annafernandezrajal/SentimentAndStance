...
plot.plotWithDiagonalMarked <- function(confusionMatrix, xlabel='Prediction', ylabel='Reference') {
  confusionMatrix_diagonalMarked <- as.data.frame(confusionMatrix$table)
  confusionMatrix_diagonalMarked$diag <- confusionMatrix_diagonalMarked[ , xlabel] == confusionMatrix_diagonalMarked[ , ylabel] # Get the diagonal
  ...
  confusionMatrix_diagonalMarked[ , ylabel] <- reverse.levels(confusionMatrix_diagonalMarked[ , ylabel]) # diagonal starts at top left
  ...
  # Some weird tricks needed to use a variable for naming the axis labels (that's why there's !! in front of the label variables in aes()
  xlabel <- ensym(xlabel)
  ylabel <- ensym(ylabel)

  # plotting the matrix
  confusionMatrix_diagonalMarked_plot <- ggplot(data = confusionMatrix_diagonalMarked, aes(x = !!xlabel, y = !!ylabel, fill = Freq)) +
...
#
# confusionMatrix <- caret::confusionMatrix(ground_sentiment_stance$Prediction, ground_sentiment_stance$Reference)
#
# plot <- plot.plotWithDiagonalMarked(confusionMatrix)
# plot
# ggsave("C:/Users/steem/Desktop/StudiesKTH/II2202 Research Methodology and Scientific Writing/SentimentAndStance/Data Analysis/images/GroundTruthSentimentStance.png")
```
